...
def rect_prism(x_range, y_range, z_range):
    ...
    print(xx)
    ax.plot_wireframe(xx, yy, z_range[0]*np.ones(yy.shape), color="r")
    ax.plot_surface(xx, yy, z_range[0]*np.ones(yy.shape), color="r", alpha=0.2)
    ax.plot_wireframe(xx, yy, z_range[1]*np.ones(yy.shape), color="r")
    ax.plot_surface(xx, yy, z_range[1]*np.ones(yy.shape), color="r", alpha=0.2)
